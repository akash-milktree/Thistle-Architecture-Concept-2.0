...
//
// Deliberately not a client component beyond the brief button, and `tier`
// (from the Stripe success_url) only selects copy: the payment is confirmed by
// the Stripe webhook, not by the browser arriving here or by which tier string
// is in the URL. Anyone can visit this page with any tier value, so it must
// never be the thing that marks a payment as complete.

const NEXT_STEPS: Record<FeasibilityTier, { title: string; body: string }[]> = {
  architectural: [
    {
...
const HEADLINE: Record<FeasibilityTier, { eyebrow: string; h1: string; lede: string }> = {
  architectural: {
...
export const FeasibilityConfirmedPage: React.FC<{ tier: FeasibilityTier }> = ({ tier }) => {
  const steps = NEXT_STEPS[tier];
  const headline = HEADLINE[tier];
    ...
    <section className="px-fl-margin py-fl-section-lg bg-thistle-white min-h-[70vh]">
      <div className="max-w-[760px] mx-auto">
        <div className="flex items-center gap-fl-3 mb-fl-5 text-thistle-green">
          ...
          <span className="text-[11px] uppercase tracking-[0.2em] font-semibold">{headline.eyebrow}</span>
        </div>
        ...
        <p className="text-fluid-lg text-thistle-black/70 leading-relaxed mb-fl-6 max-w-xl">{headline.lede}</p>
